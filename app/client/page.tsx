...
export default function ClientPortal() {
  ...
  const thisWeek = leads.filter(l => (now.getTime() - new Date(l.created_at).getTime()) < 7 * 24 * 60 * 60 * 1000)
  ...
  if (!authed) return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: C.sans }}>
      <style>{`* { box-sizing:border-box; margin:0; padding:0 } body { background:${C.bg} } input::placeholder { color:${C.textLow} }`}</style>
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 36 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: C.purple, boxShadow: `0 0 10px ${C.purple}` }} />
          <span style={{ fontSize: 11, color: C.textMid, fontFamily: C.mono, letterSpacing: '0.12em' }}>DR. SPORTSTHERAPY — CLIENT PORTAL</span>
        </div>
        <h1 style={{ fontSize: 24, color: C.textHi, fontWeight: 700, marginBottom: 8 }}>Your leads dashboard</h1>
        ...
        <input
          ...
          style={{ width: '100%', background: C.panel, border: `1px solid ${error ? '#ef4444' : C.border}`, borderRadius: 12, padding: '16px', color: C.textHi, fontSize: 16, outline: 'none', fontFamily: C.sans, marginBottom: 10 }}
        />
        {error && <p style={{ fontSize: 13, color: '#ef4444', marginBottom: 12 }}>{error}</p>}
        <button onClick={login}
          style={{ width: '100%', background: C.purple, border: 'none', borderRadius: 12, padding: '16px', color: 'white', fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: C.sans }}>
          Enter portal →
        </button>
      </div>
    </div>
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: C.sans }}>
      ...
      <div style={{ padding: '20px 16px 0', maxWidth: 680, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: C.purple, boxShadow: `0 0 10px ${C.purple}` }} />
          <span style={{ fontSize: 11, color: C.textMid, fontFamily: C.mono, letterSpacing: '0.12em' }}>DR. SPORTSTHERAPY — LEADS</span>
        </div>

        {/* Stats - 2x2 grid on mobile */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
          {[
            { label: 'Total leads', value: leads.length, color: C.purple },
            ...
            <div key={s.label} style={{ background: C.panel, border: `0.5px solid ${C.border}`, borderRadius: 12, padding: '16px', textAlign: 'center' }}>
              <div style={{ fontSize: 28, fontWeight: 700, color: s.color, fontFamily: C.mono }}>{s.value}</div>
              <div style={{ fontSize: 11, color: C.textLow, marginTop: 4, letterSpacing: '0.06em' }}>{s.label.toUpperCase()}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: '0 16px 100px', maxWidth: 680, margin: '0 auto' }}>
        ...
        {!loading && leads.length === 0 && (
          <div style={{ background: C.panel, border: `0.5px solid ${C.border}`, borderRadius: 12, padding: '32px 20px', textAlign: 'center' }}>
          ...
          {leads.map(lead => {
            ...
            return (
              <div key={lead.id}
                ...
                style={{ background: C.panel, border: `0.5px solid ${isOpen ? color : C.border}`, borderRadius: 14, padding: '16px', cursor: 'pointer', transition: 'border-color 0.15s' }}>

                {/* Lead row */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: `${color}20`, border: `1.5px solid ${color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
                    {PATTERN_ICONS[pattern]}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, color: C.textHi, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{lead.name}</div>
                    <div style={{ fontSize: 11, color, fontFamily: C.mono, marginTop: 2 }}>{PATTERN_NAMES[pattern]}</div>
                  </div>
                  <span style={{ fontSize: 11, color: C.textLow, fontFamily: C.mono, flexShrink: 0 }}>{timeAgo(lead.created_at)}</span>
                </div>
                  ...
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: `0.5px solid ${C.border}` }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
                      <a href={`mailto:${lead.email}`}
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.bg, border: `0.5px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', color: C.textMid, textDecoration: 'none', fontSize: 14 }}>
                      ...
                      {lead.whatsapp && (
                        <a href={`https://wa.me/${lead.whatsapp.replace(/\D/g, '')}`}
                          ...
                          onClick={e => e.stopPropagation()}
                          style={{ display: 'flex', alignItems: 'center', gap: 12, background: `${C.green}15`, border: `0.5px solid ${C.green}40`, borderRadius: 10, padding: '12px 14px', color: C.green, textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>
                    ...
                    {lead.metadata?.patternScores && (
                      <div>
                        ...
                        {Object.entries(lead.metadata.patternScores).sort((a, b) => b[1] - a[1]).map(([pid, s]) => {
                          ...
                          return (
                            <div key={pid} style={{ marginBottom: 7 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                                <span style={{ fontSize: 11, color: pid === pattern ? PATTERN_COLORS[pid] : C.textLow }}>{PATTERN_NAMES[pid]}</span>
